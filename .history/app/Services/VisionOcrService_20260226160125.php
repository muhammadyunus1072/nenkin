...
class VisionOcrService
{
    ...
    public function handleDocument($id, $path)
    {
        ...
        $this->detectDocumentText(storage_path('app/public/' . $folder . '/date.png'), 'date');

        // Crop Payment Top
        $coord = ['x' => 610, 'y' => 50, 'width' => 150, 'height' => 50];
        $this->cropImage($path, $coord, $folder, 'payment_top');
        $this->detectDocumentText(storage_path('app/public/' . $folder . '/payment_top.png'), 'payment_top');

        // Crop Payment
        $coord = ['x' => 610, 'y' => 370, 'width' => 180, 'height' => 50];
        $this->cropImage($path, $coord, $folder, 'payment');
        $this->detectDocumentText(storage_path('app/public/' . $folder . '/payment.png'), 'payment');

        // Crop Income
        $coord = ['x' => 610, 'y' => 440, 'width' => 180, 'height' => 50];
        $this->cropImage($path, $coord, $folder, 'income');
        $this->detectDocumentText(storage_path('app/public/' . $folder . '/income.png'), 'income');

        // Crop Net
        $coord = ['x' => 610, 'y' => 530, 'width' => 180, 'height' => 50];
        $this->cropImage($path, $coord, $folder, 'net');
        $this->detectDocumentText(storage_path('app/public/' . $folder . '/net.png'), 'net');

        // Crop Number
        $coord = ['x' => 660, 'y' => 950, 'width' => 170, 'height' => 50];
        $this->cropImage($path, $coord, $folder, 'number');
        $this->detectDocumentText(storage_path('app/public/' . $folder . '/number.png'), 'number');

        // Crop Name
        $coord = ['x' => 420, 'y' => 1000, 'width' => 700, 'height' => 50];
        $this->cropImage($path, $coord, $folder, 'name');
        $this->detectDocumentText(storage_path('app/public/' . $folder . '/name.png'), 'name');

        // Crop Address
        $coord = ['x' => 400, 'y' => 1050, 'width' => 700, 'height' => 100];
        $this->cropImage($path, $coord, $folder, 'address');
        $this->detectDocumentText(storage_path('app/public/' . $folder . '/address.png'), 'address');
    ...
    {
        // dd($path, file_exists($path), is_file($path), is_dir($path));
        $client = new ImageAnnotatorClient([
            'credentials' => base_path(env('GOOGLE_CLOUD_CREDENTIALS')),
            'transport' => 'rest',
        ]);
        // nenkin/4fd5fb4e-3f92-4e6d-a5c1-3c003f00232f/q6HQZUIfTeSamxt0BDBaXofmBPhE69HnvCsup1o2.jpg

        // Read image content
        $imageContent = file_get_contents($path);
        dd(
            strlen($imageContent),
            substr($imageContent, 0, 10)
        );
        logger('filenya = ' . $imageContent);
